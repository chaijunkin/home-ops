# Cilium Load Balancer IPAM

## IP Address Allocation

### Ranges
- ** httproute Internal**: `10.10.30.25/27`
- **httproute External**: `10.10.30.26/27`
- **k8s-Gateway**: `10.10.30.27/27`

### HTTPRoute Services (auto-generated)

#### External services
- `auth.cloudjur.com` - security/authentik-server (parent: envoy-external)
- `books.cloudjur.com` - media/calibre-web-automated (parent: envoy-external)
- `chat.cloudjur.com` - ai/open-webui (parent: envoy-external)
- `drive.cloudjur.com` - media/opencloud (parent: envoy-external)
- `echo-server-ext-auth.cloudjur.com` - network/echo-server-ext-auth (parent: envoy-external)
- `echo-server-ext.cloudjur.com` - network/echo-server-ext (parent: envoy-external)
- `flux-webhook.cloudjur.com` - flux-system/github-webhook (parent: envoy-external)
- `gatus.cloudjur.com` - observability/gatus (parent: envoy-external)
- `grafana.cloudjur.com` - observability/grafana-httproute (parent: envoy-external)
- `hass.cloudjur.com` - home-automation/home-assistant-app (parent: envoy-external)
- `home-assistant.cloudjur.com` - home-automation/home-assistant-app (parent: envoy-external)
- `jellyfin.cloudjur.com` - media/jellyfin (parent: envoy-external)
- `komga.cloudjur.com` - media/komga (parent: envoy-external)
- `kromgo.cloudjur.com` - observability/kromgo (parent: envoy-external)
- `plex.cloudjur.com` - media/plex (parent: envoy-external)
- `requests.cloudjur.com` - media/seerr (parent: envoy-external)
- `seerr.cloudjur.com` - media/seerr (parent: envoy-external)
- `share.cloudjur.com` - default/picoshare (parent: envoy-external)
- `wiki.cloudjur.com` - storage/garage-wiki-web (parent: envoy-external)

#### Internal services
- `adguard.cloudjur.com` - network/adguard (parent: envoy-internal)
- `alertmanager.cloudjur.com` - observability/kube-prometheus-stack-alertmanager (parent: envoy-internal)
- `alloy-logs.cloudjur.com` - observability/alloy-logs (parent: envoy-internal)
- `alloy-profiles.cloudjur.com` - observability/alloy-profiles (parent: envoy-internal)
- `alloy-receiver.cloudjur.com` - observability/alloy-receiver (parent: envoy-internal)
- `alloy-singleton.cloudjur.com` - observability/alloy-singleton (parent: envoy-internal)
- `atuin.cloudjur.com` - default/atuin (parent: envoy-internal)
- `autobangumi.cloudjur.com` - downloads/autobangumi (parent: envoy-internal)
- `autobrr.cloudjur.com` - downloads/autobrr (parent: envoy-internal)
- `bazarr.cloudjur.com` - downloads/bazarr (parent: envoy-internal)
- `convertx.cloudjur.com` - default/convertx (parent: envoy-internal)
- `cyberchef.cloudjur.com` - default/cyberchef (parent: envoy-internal)
- `dl-books.cloudjur.com` - downloads/shelfmark (parent: envoy-internal)
- `echo-server-int.cloudjur.com` - network/echo-server-int (parent: envoy-internal)
- `filebrowser.cloudjur.com` - media/filebrowser (parent: envoy-internal)
- `flux-operator.cloudjur.com` - flux-system/flux-operator (parent: envoy-internal)
- `frigate.cloudjur.com` - home-automation/frigate (parent: envoy-internal)
- `g.cloudjur.com` - default/glance (parent: envoy-internal)
- `garage-ui.cloudjur.com` - storage/garage-webui (parent: envoy-internal)
- `garage.cloudjur.com` - storage/garage-api (parent: envoy-internal)
- `git.cloudjur.com` - dev/forgejo (parent: envoy-internal)
- `glance.cloudjur.com` - default/glance (parent: envoy-internal)
- `h.cloudjur.com` - default/homepage (parent: envoy-internal)
- `hass-code.cloudjur.com` - home-automation/home-assistant-codeserver (parent: envoy-internal)
- `headlamp.cloudjur.com` - flux-system/headlamp (parent: envoy-internal)
- `homepage.cloudjur.com` - default/homepage (parent: envoy-internal)
- `it-tools.cloudjur.com` - default/it-tools (parent: envoy-internal)
- `kopia.cloudjur.com` - volsync-system/kopia (parent: envoy-internal)
- `lidarr.cloudjur.com` - downloads/lidarr (parent: envoy-internal)
- `maintainerr.cloudjur.com` - media/maintainerr (parent: envoy-internal)
- `mcp-direct.cloudjur.com` - ai/mcp-gateway-internal (parent: envoy-internal)
- `mcp.cloudjur.com` - ai/mcp-gateway (parent: envoy-internal)
- `metube.cloudjur.com` - downloads/metube (parent: envoy-internal)
- `money.cloudjur.com` - default/sure (parent: envoy-internal)
- `n8n-webhook.cloudjur.com` - default/n8n-webhooks (parent: envoy-internal)
- `n8n.cloudjur.com` - default/n8n-app (parent: envoy-internal)
- `ollama.cloudjur.com` - ai/ollama (parent: envoy-internal)
- `omada.cloudjur.com` - network/omada-controller (parent: envoy-internal)
- `profilarr.cloudjur.com` - downloads/profilarr (parent: envoy-internal)
- `prometheus.cloudjur.com` - observability/kube-prometheus-stack-prometheus (parent: envoy-internal)
- `prowlarr.cloudjur.com` - downloads/prowlarr (parent: envoy-internal)
- `qb.cloudjur.com` - downloads/qbittorrent-app (parent: envoy-internal)
- `qbittorrent.cloudjur.com` - downloads/qbittorrent-app (parent: envoy-internal)
- `qbop.cloudjur.com` - downloads/qbittorrent-qbop (parent: envoy-internal)
- `qui.cloudjur.com` - downloads/qui (parent: envoy-internal)
- `radarr.cloudjur.com` - downloads/radarr (parent: envoy-internal)
- `rclone.cloudjur.com` - storage/rclone (parent: envoy-internal)
- `renovate.cloudjur.com` - renovate/renovate-operator (parent: envoy-internal)
- `s3.cloudjur.com` - storage/garage-s3 (parent: envoy-internal)
- `sab.cloudjur.com` - downloads/sabnzbd (parent: envoy-internal)
- `sabnzbd.cloudjur.com` - downloads/sabnzbd (parent: envoy-internal)
- `schema.cloudjur.com` - storage/garage-schema-web (parent: envoy-internal)
- `search.cloudjur.com` - default/searxng (parent: envoy-internal)
- `slskd.cloudjur.com` - media/slskd (parent: envoy-internal)
- `sonarr.cloudjur.com` - downloads/sonarr (parent: envoy-internal)
- `switch.cloudjur.com` - network/switch (parent: envoy-internal)
- `syncthing.cloudjur.com` - media/syncthing (parent: envoy-internal)
- `tautulli.cloudjur.com` - media/tautulli (parent: envoy-internal)
- `watchstate.cloudjur.com` - media/watchstate (parent: envoy-internal)
- `webhook.cloudjur.com` - downloads/webhook (parent: envoy-internal)

#### Reserved IPs (excerpt)
```


- 10.10.30.8 - database/dragonfly (port: unknown)
- 10.10.30.9 - media/syncthing-listen (port: unknown)
- 10.10.30.11 - database/postgres-lb (port: 5432)
- 10.10.30.12 - database/mosquitto (port: 1883)
- 10.10.30.13 - downloads/qbittorrent-bittorrent (port: unknown)
- 10.10.30.14 - media/plex (port: 32400)
- 10.10.30.15 - media/filebrowser (port: 80)
- 10.10.30.16 - home-automation/frigate (port: 5000)
- 10.10.30.17 - media/jellyfin (port: 8096)
- 10.10.30.18 - media/seerr (port: 80)
- 10.10.30.19 - media/calibre-web-automated (port: 8083)
- 10.10.30.20 - default/rustdesk-server (port: 21114)
- 10.10.30.21 - default/rustdesk-relay (port: 21117)
- 10.10.30.25 - network/envoy-internal (port: 22)
- 10.10.30.26 - network/envoy-external (port: 80)
- 10.10.30.30 - kube-system/kube-api (port: 6443)
- 10.10.30.38 - ai/ollama (port: 11434)
- 10.10.30.39 - default/it-tools (port: 80)
- 10.10.30.43 - downloads/lidarr (port: 80)
- 10.10.30.44 - downloads/webhook (port: 80)
- 10.10.30.45 - ai/open-webui (port: 8080)
- 10.10.30.46 - default/searxng (port: 8080)
- 10.10.30.47 - downloads/autobrr (port: 80)
- 10.10.30.50 - home-automation/home-assistant-app (port: 8123)
- 10.10.30.52 - downloads/autobangumi (port: 7892)
- 10.10.30.53 - downloads/qbittorrent (port: 4567)
- 10.10.30.53 - downloads/qbittorrent (port: 8080)
- 10.10.30.54 - default/cyberchef (port: 8000)
- 10.10.30.57 - volsync-system/kopia (port: 80)
- 10.10.30.58 - storage/garage-webui (port: 3909)
- 10.10.30.59 - storage/garage (port: 3900)
- 10.10.30.59 - storage/garage (port: 3902)
- 10.10.30.59 - storage/garage (port: 3903)
- 10.10.30.69 - flux-system/headlamp (port: 80)
- 10.10.30.77 - media/slskd-soulseek (port: unknown)
```
